...
const calcRelatedPosts = (hexo) =>
    function () {
        ...
        const enabled = (config.enabled || config.enabled === undefined) &&
            (config.enable_env_name !== undefined ? hexo.env.args[config.enable_env_name] : true);

        if (enabled) {
            const threshold = config.filter_threshold || 0.2;
            const maxPostCount = config.related_count || 5;
            const weights = config.weight || {};
            const stemmersList = config.stemmers || [en, ru];
            ...
            const posts = hexo.locals.get('posts').data
                .map(({ title, keywords, description, raw, tags, categories, path }) => ({
                    title, keywords, description, raw, path,
            ...
            for (const { path: path1 } of posts) {
                for (const { path: path2 } of posts) {
                    if (path1 !== path2 && !matrixHasValue(postsMatrix, path1, path2)) {
                        const { title: title1, description: description1, keywords: keywords1, tags: tags1, categories: categories1 } = postsDictionary[path1];
                        const { title: title2, description: description2, keywords: keywords2, tags: tags2, categories: categories2 } = postsDictionary[path2];
                        ...
                        const value = titleCompare * (weights.title || 0.05) + descriptionCompare * (weights.description || 0.05) + keywordsCompare * (weights.keywords || 0.01) + tagsCompare * (weights.tags || 0.005) + categoriesCompare * (weights.categories || 0.005) + tfIdfCompare * (weights.text || 1);
